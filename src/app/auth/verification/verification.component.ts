...
export class VerificationComponent implements OnInit {
  ...
  checkInputLength(): void {
    ...
    if(this.firstControl.value.length === 3 && this.secondControl.value.length === 3){
      this.sendVerification();
    }
  }

  sendVerification(): void {
    const userCode = this.firstControl.value.toString() + this.secondControl.value.toString();
    const token = localStorage.getItem('token');
    this.authService.verificateUser({userCode, token}).subscribe(res => {
