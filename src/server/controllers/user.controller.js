...
class UserController {
  ...
  async addUser(req, res) {
    try {
      ...
      await conn.query("select * from users where email = ?", [email]).then(data => {
        if (data[0].length > 0) {
      ...
      conn.query('Insert users(email) values (?)', [email]).then(data => {
        conn.query('Insert customers(idUser) values(?)', [data[0].insertId]).then(data1 => {
          conn.query('Insert dayslist(idCustomer) values(?)', [data1[0].insertId]).then(data2 => {
            const token = jwt.sign(
              {email, verificationCode: '', data: {idUsers: data[0].insertId, email, admin: 0}},
              'key',
              {
                expiresIn: "2h",
              }
  ...
  async getUser(req, res) {
    try {
      ...
      if (!email) {
       ...
       conn.query("select * from users where email = ?", [email]).then( async data => {
        if(data[0].length > 0) {
